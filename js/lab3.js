...
anchorElements.forEach(anchor => {
    anchor.addEventListener('click', event => {
        ...
        if (!anchor.classList.contains('open')) {
            anchorElements.forEach(anchorCopy => {anchorCopy.classList.remove('open')}) // Remove class="open" for all list
            ...
            tabpanels.forEach(tabpanel => {
                ...
                if(anchor.hash == '#' + tabpanel.id) {
                    tabpanels.forEach(tabpanelCopy => {tabpanelCopy.style.display = 'none';})
